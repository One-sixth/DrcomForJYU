...
void MD5Transform(UINT4 [4], UCHAR[64]);
void Encode(PUCHAR, UINT4*, UINT4);
void Decode(UINT4 *, PUCHAR, UINT4);
void MD5_memcpy(PUCHAR, PUCHAR, UINT4);
void MD5_memset(PUCHAR, int, UINT4);
...
void Encode (PUCHAR output, UINT4 *input,UINT4 len)
{
  UINT4 i, j;
...
void Decode (UINT4 *output, PUCHAR input, UINT4 len)
{
  UINT4 i, j;
  for (i = 0, j = 0; j < len; i++, j += 4)
...
void MD5_memcpy (PUCHAR output,PUCHAR input, UINT4 len)
{
  UINT4 i;
  for (i = 0; i < len; i++)
   output[i] = input[i];
...
void MD5_memset (PUCHAR output,int value,UINT4 len)
{
  UINT4 i;
  for (i = 0; i < len; i++)
...
string MD5_CodeString(string Input)
{
  char Result[33]={0};
  int Len=Input.size();
  ...
  MD5_CTX  mc;
  MD5Init(&mc);
  MD5Update(&mc,(PUCHAR)Input.c_str(),Len);
  MD5Final(dst,&mc);
...
//---------------------------------------------------------------------------
//string __fastcall MD5_CodeFile(string FileName)
//{
//  if(!FileExists(FileName)) return string("");
//  int Len=0;
//  unsigned char dst[16];
//  char res[33]={0};
//  char tmp[3];
